...
logo_font = ['Streamster','Streetwear','Sabo-Regular','RoadRage-Regular','Redressed_Regular','BERNIER Distressed','BERNIER Shade','West Side','still time','Thunderstorm','Alt_Retro_Light','Pirata_One_Regular']
logo_font_link = [
...
async def lang(event):
    ...
    if not input_str:
        await edit_delete(event,f"**Available font names are here:-**\n\n{font_name}",time=60)
        return
    if input_str not in logo_font:
        catevent = await edit_or_reply(event, "`Give me a correct font name...`")
        await asyncio.sleep(1)
        await edit_delete(catevent,f"**Available font names are here:-**\n\n{font_name}",time=60)
    else:
        string = data[input_str]
        if os.path.exists("temp/logo.ttf"):
            os.remove("temp/logo.ttf")
            urllib.request.urlretrieve(string,"temp/logo.ttf",)
        await edit_delete(event, f"**Font for logo changed to :-** `{input_str}`",time=10)
...
async def lang(event):
    ...
    input_str = event.pattern_match.group(1)
    if not input_str:
        await edit_delete(event,f"**Available foreground color names are here:-**\n\n{fg_list}",time=20)
        return
    if input_str not in fg_name:
        catevent = await edit_or_reply(event, "`Give me a correct foreground color name...`")
        await asyncio.sleep(1)
        await edit_delete(catevent,f"**Available foreground color names are here:-**\n\n{fg_list}",time=20)
    else:
        addgvar("FOREGROUND", input_str)
        await edit_delete(event, f"**Foreground color for logo changed to :-** `{input_str}`",time=10)
...
async def lang(event):
    ...
    if cmd == "cbg":
        if input_str.startswith("https://telegra.ph"):
            ...
            await edit_delete(event, f"**Background for logo changed to :-** `{input_str}`",time=10)
            return
        else:
    ...
    if not input_str:
        await edit_delete(event,f"**Available background names are here:-**\n\n{bg_name}",time=10)
        return
    if input_str not in bg_name:
        catevent = await edit_or_reply(event, "`Give me a correct background name...`")
        await asyncio.sleep(1)
        await edit_delete(catevent,f"**Available background names are here:-**\n\n{bg_name}",time=10)
    else:
        string = data[input_str]
...
async def adityalogo(message):
    ...
    if gvarstatus("BACKGROUND") is None:
        urllib.request.urlretrieve("https://telegra.ph/file/47fce894d66ae67e3665f.jpg","temp/bg_img.jpg")
    else:
